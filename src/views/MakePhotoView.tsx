...
const defaultProductPackage: ProductPackage =
  constants.productPackages.find((pkg) => pkg.isPopular) ??
...
function MakePhotoView() {
  ...
  const handleFileSelect = async (
    ...
    try {
      // Convert iPhone HEIC/HEIF to JPEG when possible; fall back to original
      ...
      setPaymentMessage(
        () =>
          error instanceof Error
            ? error.message
            : "Payment failed unexpectedly. Check the console for details.",
      );
  ...
  useEffect(() => {
    ...
    setSelectedSpec(spec);
    setSpecOptions((prev) => {
      if (prev.some((opt) => opt.specCode === spec.specCode)) {
        return prev;
      }
      return [copyPhotoSpec(spec), ...prev.map(copyPhotoSpec)];
    });
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const matched = constants.productPackages.find((pkg) => pkg.id === pkgId);
    if (matched) {
                  ...
                  {processedPhoto ? (
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="text-center">
                        <button
                          ...
                          className="inline-flex items-center space-x-2 text-sm text-gray-600 hover:text-emerald-600 transition-colors underline"
                        >
                          {showOriginal ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <button
                        ...
                        className="bg-emerald-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center space-x-2"
                      >
                        {isProcessing ? (
                          <>
